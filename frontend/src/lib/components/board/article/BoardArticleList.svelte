...
<script lang="ts">
  ...
  let selectedOption = $state('title'); // 기본값 설정
    ...
    <TableBody>
      <TableBodyRow class="text-base">
        ...
        <TableBodyCell class="p-4 whitespace-normal break-all responsive-hide">
          <button onclick={editArticle} class="text-blue-300 hover:underline">수정</button> |
          <button onclick={deleteArticle} class="text-red-300 hover:underline">삭제</button>
        </TableBodyCell>
      </TableBodyRow>
    </TableBody>
  </Table>
