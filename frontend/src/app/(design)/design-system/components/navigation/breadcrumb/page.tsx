  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">사용자의 현재 위치를 보여주고 상위 페이지로 이동할 수 있게 돕는 경로 기반 네비게이션입니다.</p>
      </div>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="bg-muted p-4 rounded-lg">
            <div className="flex items-center justify-between mb-2">
              ...
              <Button variant="ghost" size="sm" onClick={() => copyToClipboard(BASIC_BREADCRUMB_CODE, 'breadcrumb-basic')}>
                {copied === 'breadcrumb-basic' ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
              </Button>
            </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>BreadcrumEllipsis를 사용하면 긴 경로의 중간 단계를 축약하여 표시할 수 있습니다.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="space-y-4">
          ...
          <div className="bg-muted p-4 rounded-lg">
            <div className="flex items-center justify-between mb-2">
              <h4 className="font-medium text-foreground">코드 예시</h4>
              <Button variant="ghost" size="sm" onClick={() => copyToClipboard(LONG_BREADCRUMB_CODE, 'breadcrumb-ellipsis')}>
                {copied === 'breadcrumb-ellipsis' ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
              </Button>
            </div>
            <pre className="text-sm overflow-x-auto">
