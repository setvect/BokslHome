    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <section className="space-y-4 rounded-3xl border border-border bg-card p-6 shadow-sm">
        ...
        <div className="space-y-2">
          ...
          <Input id="board-attachments" type="file" multiple onChange={handleAttachmentChange} className="h-11 w-full rounded-xl text-sm" />
      ...
      <footer className="flex flex-col-reverse gap-4 border-t border-border pt-6 md:flex-row md:items-center md:justify-between">
        <Button asChild variant="outline" className="h-12 min-w-[120px] rounded-xl px-6 text-base font-semibold">
          <Link href={`/board/${category.code}`}>취소</Link>
        </Button>
        <div className="flex gap-3">
          <Button type="submit" className="h-12 min-w-[120px] rounded-xl text-base font-semibold">
            확인
          </Button>
        </div>
      </footer>
