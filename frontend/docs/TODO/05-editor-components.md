...
- [ ] **번들 최적화 전략**:
  - [ ] Mermaid 동적 import: `const mermaid = await import('mermaid')`
  - [ ] Prism.js 언어별 동적 로딩
...
- [ ] Markdown Editor, HTML Editor, Editor Test 링크 추가
- [ ] navigation 배열에 에디터 메뉴 아이템 추가

### C.2 통합 테스트 페이지
- [ ] `/design-system/components/editors/test/page.tsx` 생성
- [ ] 에디터 통합 테스트 페이지 구성
- [ ] Tabs로 두 에디터 비교/변환 테스트/성능 테스트 구성
- [ ] 그리드 레이아웃으로 두 에디터 동시 비교 영역
- [ ] Markdown ↔ HTML 변환 기능 구현
- [ ] 성능 테스트 (대용량 텍스트, 변환 시간 측정)

### C.3 최종 점검 및 번들 최적화 검증
- [ ] **Next.js 번들 분석 및 최적화**:
  - [ ] `npm install --save-dev @next/bundle-analyzer`
  - [ ] Bundle analyzer 실행하여 에디터 관련 chunk 크기 확인
  - [ ] Tree-shaking 효과 검증
  - [ ] 초기 로딩 시간 측정 및 개선
- [ ] TypeScript 오류 해결
- [ ] ESLint 경고 해결
